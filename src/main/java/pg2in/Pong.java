...
public class Pong extends SimpleGraphicsApp {
    ...
    public void initialize() {
        ...
        leftPaddle = add(new Rectangle(MARGIN, HEIGHT / 2 - PAD_HEIGHT / 2, PAD_WIDTH, PAD_HEIGHT, Colors.RED));
        rightPaddle = add(new Rectangle(WIDTH - MARGIN - PAD_WIDTH, HEIGHT / 2 - PAD_HEIGHT / 2, PAD_WIDTH, PAD_HEIGHT, Colors.BLUE));
    }
    ...
    public void onKeyPressed(KeyPressedEvent event) {
        switch (event.getKeyChar()) {
            case 'w':
                leftPaddle.move(0, -PAD_MOVE_STEP); // TODO scale step by re-press
                break;
            case 's':
                leftPaddle.move(0, PAD_MOVE_STEP);
                break;
            case 'o':
                rightPaddle.move(0, -PAD_MOVE_STEP);
                break;
            case 'l':
                rightPaddle.move(0, PAD_MOVE_STEP);
                break;
